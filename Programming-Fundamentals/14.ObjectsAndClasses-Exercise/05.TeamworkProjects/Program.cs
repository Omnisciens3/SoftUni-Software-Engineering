    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < teamCount; i++)
            {
                ...
                bool doesTeamNameExist = teams.Select(x => x.TeamName).Contains(teamName);
                bool doesCreatorNameExists = teams.Select(x => x.CreatorName).Contains(creatorName);


                if (!doesTeamNameExist)
                {
                    if (!doesCreatorNameExists)
                    {
                        teams.Add(team);
            ...
            {
                string[] cmdArgs = teamMembers.Split("->");
                string newUser = cmdArgs[0];
                string teamName = cmdArgs[1];

                bool doesTeamExists = teams.Select(x => x.TeamName).Contains(teamName);
                bool doesCreatorExists = teams.Select(x => x.CreatorName).Contains(newUser);
                bool doesMemberExists = teams.Select(x => x.Members).Any(x => x.Contains(newUser));

                if (!doesTeamExists)
                {
                    Console.WriteLine($"Team {teamName} does not exist!");
                }
                else if (doesCreatorExists || doesMemberExists)
                {
